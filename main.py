...
if not (AZURE_ENDPOINT and AZURE_KEY and AZURE_REGION):
    sys.exit("❌ Define AZURE_TRANSLATOR_ENDPOINT, AZURE_TRANSLATOR_KEY y AZURE_TRANSLATOR_REGION")
...
def read_records(sh):
    ...
    for row in vals[h + 1:]:
        if not any(safe(c) for c in row):
            continue
        yield {
            "hotel": safe(row[idx.get("hotel", -1)]),
            "room" : safe(row[idx.get("habitacion", -1)]),
            "a"    : safe(row[idx.get("respuesta", -1)]),
            "media": [{
                "type": safe(row[idx.get("media_type", -1)]),
                "url" : safe(row[idx.get("media_url", -1)])
            }] if safe(row[idx.get("media_type", -1)]) and safe(row[idx.get("media_url", -1)]) else [],
        }
...
embs = openai.embeddings.create(model="text-embedding-3-small", input=CORPUS).data
...
def retrieve_answer(q: str, hotel: Optional[str], room: Optional[str]) -> Tuple[str, List[Dict[str, str]]]:
    subset = QA_ROWS
    if hotel:
    ...
    embs2 = openai.embeddings.create(model="text-embedding-3-small", input=[normalize(q)] + cand).data
...
def ask_bot(p: AskPayload):
    ...
    text_es        = restyle(raw)
    text_out       = azure_translate(text_es, to=src_lang)[0] if src_lang != "es" else text_es
...
print("Rutas cargadas:", [r.path for r in app.routes if isinstance(r, APIRoute)])
